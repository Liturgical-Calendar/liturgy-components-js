...
export default class ApiClient {
    ...
    constructor(base) {
        if (false === base instanceof ApiBase) {
            throw new Error(
                'ApiClient: the base passed to the constructor must be an ApiBase. Use ApiClient.init() to obtain a client, rather than constructing one directly.',
            );
        }
        this.#base = base;
    ...
    #assertRiteSupported() {
        if (
            this.#currentRite !== Rite.ROMAN &&
            false === this.#base.supportsRite
        ) {
            throw new Error(
                `ApiClient: the API at ${this.#base.url} does not support the ${this.#currentRite} rite. Rite support was added in API v6; this API announces no ambrosian_calendars in its metadata.`,
            );
        }
    }
    ...
    #emitCalendarFetchFailed(apiError, requestRite) {
        ...
        this.#eventBus.emit('calendarFetchFailed', apiError, {
            rite: requestRite,
        });
    }
    ...
    fetchCalendar(locale = null) {
        ...
        try {
            ...
            if (locale !== null) {
                ...
                const canonicalLocale = canonicalizeLocale(
                    locale,
                ...
                if (
                    this.#base
                        .locales()
                        .includes(new Intl.Locale(canonicalLocale).language)
                ) {
                    this.#fetchCalendarHeaders['Accept-Language'] =
                        canonicalLocale;
            ...
            const riteSegment = this.#base.supportsRite
                ? `/${requestRite}`
                : '';
                ...
                .catch((error) => {
                    ...
                    const apiError =
                        error instanceof ApiClientError
                            ? error
                            : new ApiClientError(
                                  `POST ${requestUrl} failed: ${error.message}`,
    ...
    fetchNationalCalendar(calendar_id, locale = '') {
        ...
        try {
            ...
            if (false === RiteProperties[this.#currentRite].hasNationalTier) {
                throw new Error(
                    `ApiClient.fetchNationalCalendar: the ${this.#currentRite} rite has no national calendars, so there is no route to request. Use fetchCalendar() for the rite-level calendar, or fetchDiocesanCalendar() for one of its dioceses.`,
                );
            }
            ...
            const resolvedLocale = this.#resolveCalendarLocale(
                'national',
                calendar_id,
                locale,
            );
    ...
    listenTo(uiComponent = null) {
        if (
            false === uiComponent instanceof CalendarSelect &&
            false === uiComponent instanceof ApiOptions &&
            false === uiComponent instanceof RiteSelect
        ) {
            throw new Error(
                'ApiClient.listenTo(): Expected an instance of CalendarSelect, RiteSelect or ApiOptions',
            );
        }
        ...
        if (
            uiComponent instanceof CalendarSelect ||
            uiComponent instanceof ApiOptions
        ) {
            const componentName =
                uiComponent instanceof CalendarSelect
                    ? 'CalendarSelect'
    ...
    #listenToApiOptions(apiOptions = null) {
        ...
        apiOptions._eternalHighPriestInput._domElement.addEventListener(
            'change',
            (event) => {
                this.#params.eternal_high_priest =
        ...
        apiOptions._holydaysOfObligationInput._domElement.addEventListener(
            'change',
            (event) => {
                const selectedStates = Object.fromEntries(
                    Array.from(event.target.options, (opt) => [
                        opt.value,
                        opt.selected,
                    ]),
                );
        ...
        apiOptions._yearInput._domElement.addEventListener(
            'change',
            (event) => {
    ...
    year(yearValue) {
        if (yearValue !== undefined) {
            if (
                ...
                throw new Error(
                    'year must be a number and be between 1970 and 9999',
                );
            }
    ...
    yearType(yearTypeValue) {
        if (yearTypeValue !== undefined) {
            if (
                yearTypeValue !== YearType.LITURGICAL &&
                yearTypeValue !== YearType.CIVIL
            ) {
    ...
    rite(riteValue) {
        if (false === Object.values(Rite).includes(riteValue)) {
            throw new Error(
                `ApiClient.rite: value must be a valid Rite, one of ${Object.values(Rite).join(', ')}, but found: ${String(riteValue)}`,
            );
        }
    ...
    setYear(year) {
        console.warn(
            'ApiClient.setYear() is deprecated. Use ApiClient.year() instead.',
        );
    ...
    setYearType(year_type) {
        console.warn(
            'ApiClient.setYearType() is deprecated. Use ApiClient.yearType() instead.',
        );
    ...
    static #warnAmbiguousStatic(accessor) {
        if (ApiBase.all.length > 1) {
            console.warn(
                `ApiClient.${accessor} is ambiguous: ${ApiBase.all.length} API bases are registered, and it resolved to ${ApiBase.default.url}. Read it from a specific client instead, as apiClient.base.`,
            );
        }
    }
