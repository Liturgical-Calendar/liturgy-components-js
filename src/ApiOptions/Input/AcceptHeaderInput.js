...
export default class AcceptHeaderInput extends SelectInput {
    static #RETURN_TYPE_PARAM_VALS = Object.freeze([
        'JSON',
        'XML',
        'YML',
        'ICS',
    ...
    constructor(as_return_type_param = null) {
        ...
        this._labelElement.textContent = this.#asReturnTypeParam
            ? 'return_type'
            : 'Accept Header';
        if (AcceptHeaderInput.#returnTypeOptions.length === 0) {
            AcceptHeaderInput.#returnTypeOptions =
                AcceptHeaderInput.#RETURN_TYPE_PARAM_VALS.map((value) => {
        ...
        if (AcceptHeaderInput.#acceptHeaderOptions.length === 0) {
            AcceptHeaderInput.#acceptHeaderOptions =
                AcceptHeaderInput.#ACCEPT_HEADER_VALS.map((value) => {
                    const option = document.createElement('option');
                    option.textContent = value;
                    option.value = value;
                    option.selected = this._selectedValue === value;
    ...
    asReturnTypeParam(as_return_type_param = true) {
        ...
        if (typeof as_return_type_param !== 'boolean') {
            throw new Error(
                'Invalid type for as_return_type_param, must be of type boolean but found type: ' +
                    typeof as_return_type_param,
            );
        }
        this._labelElement.textContent = as_return_type_param
            ? 'return_type'
            : 'Accept Header';
        ...
        if (as_return_type_param && false === this.#asReturnTypeParamSet) {
            ...
            false === as_return_type_param &&
            false === this.#asReturnTypeParamSet
        ) {
            this._domElement.replaceChildren(
                ...AcceptHeaderInput.#acceptHeaderOptions,
            );
