...
describe('ApiBase cache', () => {
    ...
    it('round-trips stored data', () => {
        ...
        expect(base.getCached('roman|general||2026|LITURGICAL|it')).toBe(
            payload,
        );
    });

    it('keeps two bases isolated under an identical key', () => {
        const dev = ApiBase.fromMetadata(
            'http://localhost:8000',
            FULL_METADATA,
        );
        const prod = ApiBase.fromMetadata(
    ...
    it('rejects a non-positive maxEntries', () => {
        expect(() => ApiBase.cacheLimits({ maxEntries: 0 })).toThrow(
            /maxEntries/,
        );
        expect(() => ApiBase.cacheLimits({ maxEntries: -1 })).toThrow(
            /maxEntries/,
        );
    });
