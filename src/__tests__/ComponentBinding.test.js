...
describe('CalendarSelect binding', () => {
    it('reads the metadata of the base it is bound to', () => {
        ...
        const prodSelect = new CalendarSelect({
            locale: 'en',
            apiClient: prod,
        });
    ...
    it('warns on the fallback when more than one base is registered, naming its choice', () => {
        ...
        expect(warn).toHaveBeenCalledWith(
            expect.stringContaining('CalendarSelect'),
        );
        warn.mockRestore();
    });

    it('throws when no base is registered at all', () => {
        expect(() => new CalendarSelect('en')).toThrow(
...
describe('CalendarSelect leaves the base metadata unmutated', () => {
    ...
    it('does not reorder the base national calendars when sorting its own copy', () => {
        ...
        expect(
            base.nationalCalendars().map((calendar) => calendar.calendar_id),
        ).toEqual(['IT', 'US', 'VA']);
    });
    ...
    it('leaves it unmutated however many selects in however many locales read it', () => {
        const base = ApiBase.fromMetadata(DEV, FULL_METADATA);
        ['it', 'en', 'nl', 'hu'].forEach(
            (locale) => new CalendarSelect({ locale, apiClient: { base } }),
        );
        expect(
            base.nationalCalendars().map((calendar) => calendar.calendar_id),
        ).toEqual(['IT', 'US', 'VA']);
...
describe('resolveBase rejects an apiClient that is not an ApiClient', () => {
    ...
    it('rejects an object carrying no base', () => {
        expect(
            () => new CalendarSelect({ locale: 'en', apiClient: {} }),
        ).toThrow(NAMED);
    });
    ...
    it('rejects a string', () => {
        expect(
            () =>
                new CalendarSelect({ locale: 'en', apiClient: 'not-a-client' }),
        ).toThrow(NAMED);
    });
    ...
    it('rejects an ApiBase passed where an ApiClient was expected', () => {
        ...
        expect(
            () => new CalendarSelect({ locale: 'en', apiClient: base }),
        ).toThrow(NAMED);
    });
...
describe('the ambiguous fallback warns once per component', () => {
    ...
    it('warns separately for a different component class', () => {
        ...
        expect(warn).toHaveBeenCalledWith(
            expect.stringContaining('ApiOptions'),
        );
        warn.mockRestore();
    });
...
describe('ApiOptions binding', () => {
    ...
    it('offers the locales of its own base', () => {
        ...
        expect(devOptions._localeInput.options()).toEqual(
...
describe('ApiOptions keeps the bare locale string form intact', () => {
    ...
    it('applies the locale it is given rather than defaulting to English', () => {
        ...
        const labels = Array.from(
            apiOptions._localeInput._domElement.options,
...
describe('ApiOptions rejects an argument that is neither a locale string nor an options object', () => {
    const BAD_OPTIONS =
        /Invalid type for options, must be of type `object` but found type/;
    ...
    it('accepts a bare Intl.Locale as the locale, on the fallback base', () => {
        ...
        expect(apiOptions._base).toBe(ApiBase.resolve(DEV));
        const labels = [...apiOptions._localeInput._domElement.options].map(
    ...
    it('rejects a non-string, non-Intl.Locale locale inside a valid options object', () => {
        expect(() => new ApiOptions({ locale: 123 })).toThrow(
            /Invalid type for locale, must be of type `string` or `Intl.Locale`/,
        );
        expect(() => new ApiOptions({ locale: ['it'] })).toThrow(
            /Invalid type for locale/,
        );
    });

    it('still accepts an options object carrying only an apiClient', () => {
        const prod = clientFor(PROD, OTHER_METADATA);
        expect(new ApiOptions({ apiClient: prod })._base).toBe(
            ApiBase.resolve(PROD),
        );
    });
...
describe('PathBuilder binding', () => {
    ...
    it('throws when its arguments are bound to different bases, naming both', () => {
        ...
        expect(() => new PathBuilder(apiOptions, calendarSelect)).toThrow(
            /different API bases/,
        );
        expect(() => new PathBuilder(apiOptions, calendarSelect)).toThrow(
            new RegExp(DEV.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')),
        );
        expect(() => new PathBuilder(apiOptions, calendarSelect)).toThrow(
            /example\.org/,
        );
...
describe('CalendarSelect.linkToNationsSelect binding', () => {
    it('throws when the dioceses select and the nations select are bound to different bases, naming both', () => {
        const devClient = clientFor(DEV, FULL_METADATA);
        const prodClient = clientFor(PROD, OTHER_METADATA);
        const nationSelect = new CalendarSelect({
            locale: 'en',
            apiClient: devClient,
        }).filter(CalendarSelectFilter.NATIONAL_CALENDARS);
        const dioceseSelect = new CalendarSelect({
            locale: 'en',
            apiClient: prodClient,
...
describe('ApiOptions.linkToCalendarSelect binding', () => {
    it('throws when the single linked CalendarSelect is bound to another base, naming both', () => {
        ...
        expect(() => apiOptions.linkToCalendarSelect(calendarSelect)).toThrow(
            /different API bases/,
        );
        expect(() => apiOptions.linkToCalendarSelect(calendarSelect)).toThrow(
            new RegExp(DEV.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')),
    ...
    it('throws when either half of a linked nation/diocese pair is bound to another base', () => {
        ...
        const strayNation = new CalendarSelect({
            locale: 'en',
            apiClient: prodClient,
        }).filter(CalendarSelectFilter.NATIONAL_CALENDARS);
        const ownDiocese = new CalendarSelect({
            locale: 'en',
            apiClient: devClient,
        }).filter(CalendarSelectFilter.DIOCESAN_CALENDARS);
        expect(() =>
            new ApiOptions({
                locale: 'en',
                apiClient: devClient,
            }).linkToCalendarSelect([strayNation, ownDiocese]),
        ).toThrow(/different API bases/);

        const ownNation = new CalendarSelect({
            locale: 'en',
            apiClient: devClient,
        }).filter(CalendarSelectFilter.NATIONAL_CALENDARS);
        const strayDiocese = new CalendarSelect({
            locale: 'en',
            apiClient: prodClient,
        }).filter(CalendarSelectFilter.DIOCESAN_CALENDARS);
        expect(() =>
            new ApiOptions({
                locale: 'en',
                apiClient: devClient,
            }).linkToCalendarSelect([ownNation, strayDiocese]),
        ).toThrow(/different API bases/);
    });

    it('does not throw when every linked select shares the ApiOptions base', () => {
        const devClient = clientFor(DEV, FULL_METADATA);
        const nation = new CalendarSelect({
            locale: 'en',
            apiClient: devClient,
        }).filter(CalendarSelectFilter.NATIONAL_CALENDARS);
        const diocese = new CalendarSelect({
            locale: 'en',
            apiClient: devClient,
        }).filter(CalendarSelectFilter.DIOCESAN_CALENDARS);
        expect(() =>
            new ApiOptions({
                locale: 'en',
                apiClient: devClient,
            }).linkToCalendarSelect([nation, diocese]),
        ).not.toThrow();
        expect(() =>
            new ApiOptions({
                locale: 'en',
                apiClient: devClient,
            }).linkToCalendarSelect(
    ...
    it('leaves the ApiOptions unlinked when it throws', () => {
        ...
        const stray = new CalendarSelect({
            locale: 'en',
            apiClient: prodClient,
        });
        expect(() =>
            apiOptions.linkToCalendarSelect(stray, new RiteSelect('en')),
        ).toThrow(/different API bases/);
...
describe('ApiClient.listenTo binding', () => {
    it('throws when the CalendarSelect is bound to another base, naming both', () => {
        ...
        expect(() => client.listenTo(select)).toThrow(
            new RegExp(DEV.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')),
        );
    ...
    it('throws when the ApiOptions is bound to another base', () => {
        ...
        expect(() => client.listenTo(apiOptions)).toThrow(
            /different API bases/,
        );
    });
    ...
    it('does not throw for components bound to the client own base', () => {
        ...
        expect(() =>
            client.listenTo(
                new CalendarSelect({ locale: 'en', apiClient: { base: dev } }),
            ),
        ).not.toThrow();
        expect(() =>
            client.listenTo(
                new ApiOptions({ locale: 'en', apiClient: { base: dev } }),
            ),
        ).not.toThrow();
    });
...
describe('ApiBase.assertSameBase', () => {
    it('does not throw when both bases are the same', () => {
        const base = ApiBase.fromMetadata(DEV, FULL_METADATA);
        expect(() =>
            assertSameBase(base, base, 'Test: a and b', 'Nothing would break.'),
        ).not.toThrow();
    });
    ...
    it('reports both sides when neither carries a base', () => {
        expect(() =>
            ...
            /the first carries no API base \(found undefined\), and the second carries no API base \(found null\)/,
        );
    });
