...
describe('CalendarSelect', () => {
    ...
    it('throws on an unparseable locale, naming itself and the tag', () => {
        expect(() => new CalendarSelect('not a locale')).toThrow(
            'CalendarSelect: Invalid locale: not a locale',
        );
        expect(() => new CalendarSelect({ locale: 'it__IT' })).toThrow(
            'CalendarSelect: Invalid locale: it--IT',
        );
    });

    it('rejects a non-string locale, naming itself and the type', () => {
        expect(() => new CalendarSelect({ locale: 123 })).toThrow(
            'CalendarSelect: Invalid type for locale, must be of type `string` or `Intl.Locale` but found type: number',
        );
    });
    ...
    it('still defaults an absent locale to English', () => {
        ...
        expect(calendarSelectLocale(new CalendarSelect({}))).toBe('en');
        expect(
            calendarSelectLocale(new CalendarSelect({ locale: undefined })),
        ).toBe('en');
        expect(calendarSelectLocale(new CalendarSelect({ locale: null }))).toBe(
            'en',
        );
    });
    ...
    it('accepts an Intl.Locale, bare or inside a bag', () => {
        expect(
            calendarSelectLocale(new CalendarSelect(new Intl.Locale('it-IT'))),
        ).toBe('it-IT');
        expect(
            calendarSelectLocale(
                new CalendarSelect({ locale: new Intl.Locale('it-IT') }),
            ),
        ).toBe('it-IT');
    });
    ...
    it('canonicalizes an Intl.Locale built from a non-canonical tag', () => {
        expect(
            calendarSelectLocale(new CalendarSelect(new Intl.Locale('EN-us'))),
        ).toBe('en-US');
    });
});
...
describe('RiteSelect', () => {
    ...
    it('throws on an unparseable locale, naming itself and the tag', () => {
        expect(() => new RiteSelect('not a locale')).toThrow(
            'RiteSelect: Invalid locale: not a locale',
        );
    });

    it('rejects a non-string locale, naming itself and the type', () => {
        expect(() => new RiteSelect({ locale: 123 })).toThrow(
            'RiteSelect: Invalid type for locale, must be of type `string` or `Intl.Locale` but found type: number',
        );
    });
    ...
    it('accepts an Intl.Locale, bare or inside a bag', () => {
        ...
        expect(
            new RiteSelect({ locale: new Intl.Locale('it-IT') })._locale,
        ).toBe('it-IT');
...
describe('ApiOptions', () => {
    ...
    const localeLabels = (apiOptions) =>
        Array.from(apiOptions._localeInput._domElement.options).map(
    ...
    it('normalizes an underscored locale to the same result as the hyphenated form', () => {
        expect(localeLabels(new ApiOptions('it_IT'))).toEqual(
            localeLabels(new ApiOptions('it-IT')),
        );
    });

    it('throws on an unparseable locale, naming itself and the tag', () => {
        expect(() => new ApiOptions('not a locale')).toThrow(
            'ApiOptions: Invalid locale: not a locale',
        );
        expect(() => new ApiOptions('it__IT')).toThrow(
            'ApiOptions: Invalid locale: it--IT',
        );
    });

    it('rejects a non-string locale, naming itself and the type', () => {
        expect(() => new ApiOptions({ locale: 123 })).toThrow(
            'ApiOptions: Invalid type for locale, must be of type `string` or `Intl.Locale` but found type: number',
        );
    });
    ...
    it('defaults an absent locale to English, however the absence is spelled', () => {
        ...
        expect(localeLabels(new ApiOptions({}))).toContain('Italian');
        expect(localeLabels(new ApiOptions({ locale: undefined }))).toContain(
            'Italian',
        );
        expect(localeLabels(new ApiOptions({ locale: null }))).toContain(
            'Italian',
        );
    ...
    it('accepts an Intl.Locale, bare or inside a bag', () => {
        expect(
            localeLabels(new ApiOptions(new Intl.Locale('it-IT'))),
        ).toContain('italiano');
        expect(
            localeLabels(new ApiOptions({ locale: new Intl.Locale('it-IT') })),
        ).toContain('italiano');
    });
});
...
describe('WebCalendar', () => {
    ...
    it('throws on an unparseable locale, naming the setter and the tag', () => {
        expect(() => new WebCalendar().locale('not a locale')).toThrow(
            'WebCalendar.locale: Invalid locale: not a locale',
        );
        expect(() => new WebCalendar().locale('it__IT')).toThrow(
            'WebCalendar.locale: Invalid locale: it--IT',
        );
    });

    it('rejects a non-string locale, naming the setter and the type', () => {
        expect(() => new WebCalendar().locale(123)).toThrow(
            'WebCalendar.locale: Invalid type for locale, must be of type `string` or `Intl.Locale` but found type: number',
        );
    });
    ...
    it('gives the empty string its own message, from the shared helper', () => {
        expect(() => new WebCalendar().locale('')).toThrow(
            'WebCalendar.locale: Invalid locale, cannot be an empty or blank string',
        );
        expect(() => new WebCalendar().locale('   ')).toThrow(
            'WebCalendar.locale: Invalid locale, cannot be an empty or blank string',
        );
    ...
    it('accepts an Intl.Locale and stores its canonical tag', () => {
        expect(new WebCalendar().locale(new Intl.Locale('it-IT'))._locale).toBe(
            'it-IT',
        );
        expect(new WebCalendar().locale(new Intl.Locale('EN-us'))._locale).toBe(
            'en-US',
        );
    });
});
...
describe.each([
    ...
    it('stores the canonical tag for a valid locale', () => {
        expect(titleText(build('it-IT'))).toBe(
    ...
    it('normalizes an underscored locale, region subtag intact', () => {
        ...
        expect(titleText(build('it_IT'))).toBe(
            Messages['it']['LITURGY_OF_THE_DAY'],
        );
    });

    it('throws on an unparseable locale, naming itself and the tag', () => {
        expect(() => build('not a locale')).toThrow(
            `${name}: Invalid locale: not a locale`,
        );
        expect(() => build({ locale: 'it__IT' })).toThrow(
            `${name}: Invalid locale: it--IT`,
        );
    });

    it('rejects a non-string locale, naming itself and the type', () => {
        expect(() => build({ locale: 123 })).toThrow(
            `${name}: Invalid type for locale, must be of type \`string\` or \`Intl.Locale\` but found type: number`,
        );
    });
    ...
    it('defaults an absent locale to English, however the absence is spelled', () => {
        ...
        expect(titleText(build({}))).toBe(Messages['en']['LITURGY_OF_THE_DAY']);
        expect(titleText(build(null))).toBe(
            Messages['en']['LITURGY_OF_THE_DAY'],
        );
        expect(titleText(build(undefined))).toBe(
            Messages['en']['LITURGY_OF_THE_DAY'],
        );
        expect(titleText(build({ locale: null }))).toBe(
            Messages['en']['LITURGY_OF_THE_DAY'],
        );
        expect(titleText(build({ locale: undefined }))).toBe(
            Messages['en']['LITURGY_OF_THE_DAY'],
        );
    });

    it('accepts an Intl.Locale, bare or inside a bag, region subtag intact', () => {
        expect(titleText(build(new Intl.Locale('it-IT')))).toBe(
            Messages['it']['LITURGY_OF_THE_DAY'],
        );
        expect(titleText(build({ locale: new Intl.Locale('it-IT') }))).toBe(
            Messages['it']['LITURGY_OF_THE_DAY'],
        );
        expect(dateText(build(new Intl.Locale('en-GB')))).toBe(
            dateText(build('en-GB')),
        );
        expect(dateText(build(new Intl.Locale('en-GB')))).not.toBe(
            dateText(build('en-US')),
        );
    });
});
...
describe('ApiClient.fetchCalendar locale argument', () => {
    ...
    it('rejects an empty or blank locale, naming the method', async () => {
        await expect(apiClient.fetchCalendar('')).rejects.toThrow(
            'ApiClient.fetchCalendar: Invalid locale, cannot be an empty or blank string',
        );
        await expect(apiClient.fetchCalendar('   ')).rejects.toThrow(
            'ApiClient.fetchCalendar: Invalid locale, cannot be an empty or blank string',
        );
    ...
    it('rejects a non-string locale, naming the method and the type', async () => {
        await expect(apiClient.fetchCalendar(123)).rejects.toThrow(
            'ApiClient.fetchCalendar: Invalid type for locale, must be of type `string` or `Intl.Locale` but found type: number',
        );
    });
    ...
    it('emits no calendarFetchFailed for an unusable locale', async () => {
        ...
        await expect(apiClient.fetchCalendar('')).rejects.not.toBeInstanceOf(
            ApiClientError,
        );
    ...
    it('normalizes an underscored locale into the Accept-Language header', () => {
        ...
        expect(global.fetch.mock.calls[0][1].headers['Accept-Language']).toBe(
            'it-IT',
        );
    });
