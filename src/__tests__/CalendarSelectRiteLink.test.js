...
describe('CalendarSelect.linkToRiteSelect', () => {
    it('is chainable and rejects a non-RiteSelect', () => {
        ...
        expect(calendarSelect.linkToRiteSelect(riteSelect)).toBe(
            calendarSelect,
        );

        const other = new CalendarSelect('en');
        other.appendTo('#diocese');
        expect(() => other.linkToRiteSelect({})).toThrow(
    ...
    it('throws when linked to a rite select twice', () => {
        ...
        expect(() => calendarSelect.linkToRiteSelect(riteSelect)).toThrow(
            /already linked to a RiteSelect/,
        );
    });
    ...
    it('rebuilds a dioceses filtered select on a rite change and clears the selection', () => {
        ...
        const values = [...dioceseSelect._domElement.options].map(
            (o) => o.value,
        );
    ...
    it('hides a nations filtered select for a rite with no national tier, and shows it again', () => {
        ...
        nationSelect.appendTo('#nation');
        const target = () =>
            nationSelect._wrapperElement ?? nationSelect._domElement;
    ...
    it('dispatches change on a standalone nation select, but not on one with a dependent', () => {
        ...
        const paired = new CalendarSelect('en').filter(
            CalendarSelectFilter.NATIONAL_CALENDARS,
        );
        ...
        let loneChanges = 0;
        let pairedChanges = 0;
        lone._domElement.addEventListener('change', () => {
            loneChanges++;
        });
        paired._domElement.addEventListener('change', () => {
            pairedChanges++;
        });
...
describe('linkToRiteSelect and ApiOptions together', () => {
    it('throws when a select linked by ApiOptions is also linked directly', async () => {
        const { default: ApiOptions } =
            await import('../ApiOptions/ApiOptions.js');
    ...
    it('dispatches change exactly once on a linked diocese select, and not at all on its nation select', async () => {
        ...
        nationSelect._domElement.addEventListener('change', () => {
            nationChanges++;
        });
        dioceseSelect._domElement.addEventListener('change', () => {
            dioceseChanges++;
        });
