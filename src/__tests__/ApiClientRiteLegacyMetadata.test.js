...
describe('ApiClient against metadata with no ambrosian_calendars (live v5 API)', () => {
    ...
    it.each([
        ...
        [
            ...
            (client) => client.fetchNationalCalendar('IT'),
        ],
        [
        ...
        '%s rejects with a labelled error rather than emitting a request the API will reject',
        async (_name, fetchWith) => {
            apiClient.rite(Rite.AMBROSIAN);
            await expect(fetchWith(apiClient)).rejects.toThrow(
                /does not support the ambrosian rite/,
            );
