...
describe('ApiBase metadata queries', () => {
    ...
    it('returns every national calendar', () => {
        ...
        expect(base.nationalCalendars().map((c) => c.calendar_id)).toEqual([
            'IT',
            'US',
            'VA',
        ]);
    ...
    it('filters diocesan calendars by rite', () => {
        ...
        expect(
            base.diocesanCalendars(Rite.ROMAN).map((c) => c.calendar_id),
        ).toEqual(['romamo_it', 'boston_us']);
        expect(
            base.diocesanCalendars(Rite.AMBROSIAN).map((c) => c.calendar_id),
        ).toEqual(['milano_it']);
    });
    ...
    it('defaults diocesanCalendars to the Roman rite', () => {
        ...
        expect(base.diocesanCalendars()).toEqual(
            base.diocesanCalendars(Rite.ROMAN),
        );
    });
    ...
    it('returns a rite own calendars under the {rite}_calendars convention', () => {
        ...
        expect(
            base.riteCalendars(Rite.AMBROSIAN).map((c) => c.calendar_id),
        ).toEqual(['ambrosian']);
    });
    ...
    it('reports rite support from the presence of ambrosian_calendars', () => {
        expect(
            ApiBase.fromMetadata('http://a', FULL_METADATA).supportsRite,
        ).toBe(true);
        expect(ApiBase.fromMetadata('http://b', V5_METADATA).supportsRite).toBe(
            false,
        );
    });
    ...
    it('throws a query on an unloaded base rather than returning an empty result', () => {
        ...
        expect(() => base.riteCalendars(Rite.ROMAN)).toThrow(
            /has not been loaded/,
        );
        expect(() => base.isValidDioceseForNation('romamo_it', 'IT')).toThrow(
            /has not been loaded/,
        );
    });
