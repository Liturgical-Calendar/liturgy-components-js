...
describe('ApiClient rite state', () => {
    ...
    it('throws on a value that is not a Rite', () => {
        expect(() => apiClient.rite('byzantine')).toThrow(
            /must be a valid Rite/,
        );
...
describe('ApiClient national tier guard', () => {
    it('refuses a national calendar under a rite that has no national tier', async () => {
        ...
        // assertion.
        apiClient.rite(Rite.AMBROSIAN);
        await expect(apiClient.fetchNationalCalendar('IT')).rejects.toThrow(
            /has no national calendars/,
        );
...
describe('ApiClient response freshness', () => {
    ...
    it('ignores a response that a newer request has superseded', async () => {
        ...
        apiClient._eventBus.on('calendarFetched', (data) =>
            emitted.push(data.settings.year),
        );
