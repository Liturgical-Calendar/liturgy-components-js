...
describe('LiturgyOfAnyDay configuration methods are chainable', () => {
    ...
    const CHAINABLE = [
        ...
        [
            'readingsLabelClass',
            () => widget.readingsLabelClass('reading-label'),
        ],
        ['readingClass', () => widget.readingClass('reading')],
        ['showReadings', () => widget.showReadings(false)],
        [
            'dayInputConfig',
            () => widget.dayInputConfig({ class: 'form-control' }),
        ],
        [
            'monthInputConfig',
            () => widget.monthInputConfig({ class: 'form-select' }),
        ],
        [
            'yearInputConfig',
            () => widget.yearInputConfig({ class: 'form-control' }),
        ],
    ...
    it.each(CHAINABLE)(
        '%s returns the instance for chaining',
        (_name, call) => {
...
describe('LiturgyOfAnyDay renders events from a calendarFetched payload', () => {
    ...
    it('renders only the event matching the selected date, not other days on the same payload', () => {
        ...
        expect(widget._eventsElementsWrapper.textContent).toContain(
            'Saint John Mary Vianney',
        );
        expect(widget._eventsElementsWrapper.textContent).not.toContain(
            'Not Today',
        );
    });
    ...
    it('suppresses the grade text for an Ordinary Sunday event_key', () => {
        apiClient._eventBus.emit(
            ...
            buildCalendarData([
                buildEvent({
                    event_key: 'OrdSunday15',
                    name: 'Fifteenth Sunday in Ordinary Time',
                    grade: 4,
                    grade_lcl: 'Sunday',
                    grade_display: null,
                }),
        ...
        expect(
            widget._eventsElementsWrapper.firstElementChild.querySelector(
                '.grade',
            ),
        ).toBeNull();
    });
    ...
    it.each([
        ...
        apiClient._eventBus.emit(
            'calendarFetched',
            buildCalendarData([buildEvent({ color: [bgColor] })]),
        );
        const eventEl = widget._eventsElementsWrapper.firstElementChild;
    ...
    it('adds a border for a white background', () => {
        apiClient._eventBus.emit(
            'calendarFetched',
            buildCalendarData([buildEvent({ color: ['white'] })]),
        );
        ...
        expect(
            widget._eventsElementsWrapper.firstElementChild.style.border,
        ).toBe('1px solid rgb(222, 226, 230)');
    });

    it('adds no border for a non-white background', () => {
        apiClient._eventBus.emit(
            'calendarFetched',
            buildCalendarData([buildEvent({ color: ['green'] })]),
        );
        expect(
            widget._eventsElementsWrapper.firstElementChild.style.border,
        ).toBe('');
    ...
    it('renders lectionary readings when present and showReadings is enabled', () => {
        apiClient._eventBus.emit(
            ...
            buildCalendarData([
                buildEvent({
                    readings: { first_reading: 'Gn 1:1', gospel: 'Jn 1:1' },
                }),
            ]),
        );
        const readingsEl =
...
describe('LiturgyOfAnyDay.listenTo validates the shape of calendarFetched payloads', () => {
    ...
    it('throws synchronously when litcal is missing', () => {
        expect(() => apiClient._eventBus.emit('calendarFetched', {})).toThrow(
            /Invalid liturgical calendar data/,
        );
    });

    it('throws synchronously when litcal is empty', () => {
        expect(() =>
            apiClient._eventBus.emit('calendarFetched', { litcal: [] }),
...
describe('LiturgyOfAnyDay caching: day and month changes render without a refetch', () => {
    ...
    beforeEach(async () => {
        ...
        global.fetch = jest.fn().mockResolvedValue({
            ok: true,
            json: async () => buildCalendarData([buildEvent()]),
        });
    });
