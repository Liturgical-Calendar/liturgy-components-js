...
const meta = {
    ...
    argTypes: {
        locale: {
            control: 'text',
            description:
                'Locale code for UI elements, and also the locale used to fetch the Diocesan Calendar when the calendar supports more than one locale',
        },
        id: {
            control: 'text',
    ...
    render: (args, { loaded: { apiClient } }) => {
        ...
        if (!apiClient || !(apiClient instanceof ApiClient)) {
            ...
            const diocesanCalendarMetadata =
                apiClient._metadata.diocesan_calendars.find(
            ...
            if (!diocesanCalendarMetadata) {
                ...
                const locale =
                    args.locale &&
                    args.locale !== '' &&
                    diocesanCalendarMetadata.locales.includes(args.locale)
                        ? args.locale
                        : diocesanCalendarMetadata.locales[0];
                ...
                apiClient
                    .fetchDiocesanCalendar(args.calendar_id, locale)
                    .catch((error) => {
                        container.textContent =
